	...
	public void onPlayerMoveEvent(PlayerMoveEvent e) {
		...
		if (e.before.dim != player.dimension) {
			for (int i = 0; i < RestrictedPortals.idSplit.length; i++) {
				if (player.dimension == Integer.parseInt(RestrictedPortals.idSplit[i].trim()) && player.getStatFile().readStat(RestrictedPortals.portalUnlock[i]) == 0) {
					player.sendStatusMessage(new TextComponentTranslation(RestrictedPortals.blockedmessage.replace("%item%", RestrictedPortals.itemList[i].getDisplayName()).replace("%dim%",RestrictedPortals.nameSplit[i])), false);
					e.setCanceled(true);
				}
			}
